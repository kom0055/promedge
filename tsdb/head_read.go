	...
	*chks = (*chks)[:0]
	...
	if s.headChunk != nil && s.headChunk.OverlapsClosedInterval(h.mint, h.maxt) {
		*chks = append(*chks, chunks.Meta{
			...
			Ref:     chunks.ChunkRef(chunks.NewHeadChunkRef(s.ref, s.headChunkID(0))),
		})
	}
...
func (s *memSeries) iterator(id chunks.HeadChunkID, isoState *isolationState, it chunkenc.Iterator) chunkenc.Iterator {
	c, err := s.chunk(id)
