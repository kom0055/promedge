...
func NewHead(r prometheus.Registerer, l log.Logger, opts *HeadOptions) (*Head, error) {
	if l == nil {
		l = log.NewNopLogger()
	}
...
func (h *Head) Init(minValidTime int64) {
	h.minValidTime.Store(minValidTime)
	defer func() {
		h.postings.EnsureOrder()
	}()
	defer h.gc() // remove the obsolete data from the head.
	defer func() {
	...
	for i := 0; i < s.size; i++ {
		...
		for hash, all := range s.hashes[i] {
			for _, series := range all {
				...
				if series.headChunk != nil || series.pendingCommit {
