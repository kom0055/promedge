...
func (s *memSeries) append(t int64, v float64, appendID uint64) (sampleInOrder, chunkCreated bool) {
...
// todo redis rehash或者用lru
func (s *memSeries) cutNewHeadChunk(mint int64) *memChunk {
	prev := s.headChunk
	if prev == nil || prev.chunk == nil {
		...
		s.headChunk = &memChunk{
			chunk:   newBuddyWithPrev(prev.chunk.GetCurr()),
			minTime: mint,
			maxTime: math.MinInt64,
		}
	}
